...
impl Parse {
    ...
    pub (crate) fn next_string(&mut self) -> Result<String,ParseError>{
        match self.next()?{
            // Both `Simple` and `Bulk` representation may be strings, String are parsed to UTF-8
            // While errors are stored as strings, they are considered seperate
            // types
            Frame::Simple(s)=>Ok(s),
            ...
            frame => Err(format!(
                "protocol error; expected simple frame or build frame, got {:?}",
                frame
            ).into()
        ),
        }
...
/// Returns the next entry as an integer.
/// 
